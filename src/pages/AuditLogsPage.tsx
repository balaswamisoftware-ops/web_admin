...
import { Input, Pagination, useToast } from '../components/ui'
...
const FAMILIES = ['all', 'chant', 'donation', 'settings', 'devotee', 'admin'] as const
...
export function AuditLogsPage() {
  ...
  const familyFiltered = useMemo(
    () => (family === 'all' ? logs : logs.filter(l => l.action.startsWith(family))),
    [logs, family],
  )

  const table = useTableControls(familyFiltered, {
    search: (l, q) =>
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">Audit log</div>
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <label className="flex flex-col gap-1.5 text-sm font-medium text-stone-700 dark:text-stone-200">
          Type
          <select value={family} onChange={e => setFamily(e.target.value as Family)} className={selCls}>
            {FAMILIES.map(f => (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="mb-1.5 flex flex-wrap items-center gap-2">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${actionTint(log.action)}`}>
                        {log.action}
                      </span>
                      {log.revertOf && (
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      <span className="font-medium text-gray-600 dark:text-gray-300">{log.actorName}</span>
